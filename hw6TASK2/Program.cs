...
class Program
{
    ...
    static int GetInt(int max)
    {
        while (true)
            if (!int.TryParse(Console.ReadLine(), out int x) || x > max)
                Console.Write("Ошибка! Укажите число от 0 до {0}).\nПовторите ввод: ", max);
            else return x;
    }
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("Выберите функцию из предложенных и укажите номер цифрой:");
        ...
        Console.WriteLine("Задайте отрезок для нахождения минимума в формате «х1 х2» (два числа через пробел):");
